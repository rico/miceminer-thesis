...
\hline
\\ 
\lstinline|conf/conf.xml|	&	The main \ac{XML} configuration file. \\ [3ex] 
\lstinline|conf/micedata_create.sql|	&	The SQL file used to create the database structure. \\ [3ex] 
\lstinline|import_scripts//|	&	Contains the \ac{BASH} scripts to start the import cascade at a specific script (see \ref{app:import_schedule} on page \pageref{app:import_schedule} for details). \\ [3ex]
\lstinline|data/|	&	Uploaded but not yet imported data files. \\ [3ex]
\lstinline|data/cronlogs/|	&	Transcripts created the \ac{perl} scripts involved in the data import process. \\ [3ex] 
\lstinline|data/imported/|	&	After successful import of a data file into the database, the file is moved into this directory.  \\ [3ex] 
\lstinline|miceminer/|	&	The \textit{miceminer} application main files and folders.\\ [3ex]
\lstinline|perl/cgi|	&	\ac{CGI} scripts written in \ac{perl} which are used by the \textit{miceminer} application. \\ [3ex]
\lstinline|perl/extras/|	&	Perl scripts to control the data validity in the database and to compile data for some special analysis. The task of these scripts is usually explained in the header section of the file. \\ [3ex]
\lstinline|perl/import/|	&	Contains the perl script involved in the data import and processing. \\ [3ex]
\lstinline|perl/lib/|	&	The perl modules needed by all the import, reset and CGI scripts. \\ [3ex]
\lstinline|perl/reset/|	&	Scripts to set back the data in the database, so that the import scripts can be rerun. \\ [3ex]
\lstinline|amfphp/services/miceMiner/Mice.php|	&	\href{http://www.amfphp.org/}{amfphp} services file. \\ [3ex] 
...
\lstinline|import_step_1.sh|	&	\lstinline|logimport.pl| \\ [1ex] 
\lstinline|direction_step_2.sh|	&	\lstinline|searchdir.pl| \\ [1ex]
\lstinline|results_step_3.sh|	&	\lstinline|searchres.pl| \\ [1ex]
\lstinline|meetings_step_4.sh|	&	\lstinline|meetings.pl| \\ [1ex]
