...
% Font
%\usepackage{times} % uncomment for sans serif
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\usepackage{assets/fonts/garamond/garamond} % uncomment for  serif 
...
%\renewcommand{\familydefault}{\sfdefault} % uncomment for sans serif / comment for serif
...
\vspace*{\fill}
\hvFloat[rotAngle=90,nonFloat=true]{figure}
  {\includegraphics[width=.88\textheight]{assets/pdf/application_design.pdf}}
  {Overview of the application design.}{fig:app_design}
\vspace*{\fill}
...
\url{http://zool-miceminer.uzh.ch}	&	Main Site containing containing a lot of information about the project. \\ 
\url{afp://zool-miceminer.uzh.ch}	&	\ac{AFP} access to the server. \\ 
\url{smb://zool-miceminer.uzh.ch}	&	\ac{Samba} access to the server. \\
\url{http://zool-miceminer.uzh.ch/phpmyadmin}	&	Web-based MySQL database administration tool (\href{http://www.phpmyadmin.net}{phpMyAdmin}). \\
\url{http://zool-miceminer.uzh.ch/amfphp/browser/index.html}	&	\href{http://www.amfphp.org}{amfphp} service browser. \\
\url{http://zool-miceminer.uzh.ch/webalizer}	&	Webserver traffic analyzer (\href{http://www.webalizer.org}{Webalizer}). \\
\url{http://zool-miceminer.uzh.ch/miceminer/srcview/index.html}	&	Source of the \textit{miceminer} application. \\
\url{http://zool-miceminer.uzh.ch/asdoc/index.html}	& \textit{Adobe Flex} \ac{API} documentation of the \textit{miceminer} application. \\
\url{http://zool-miceminer.uzh.ch/conf/config.xml}	&	XML-based application wide configuration file (special login required). \\
\url{http://zool-miceminer.uzh.ch/conf/micedata_create.sql}	&	The SQL which are used to create the database structure (special login required). \\
\url{http://github.com/rico/miceminer-perl/tree/master}	& \ac{GIT} repository of the perl part of the application containing all the source code.\\
\url{http://github.com/rico/miceminer-thesis/tree/master}	& GIT repository of this document including all the LaTeX source files, schemata, \lstinline|R| files etc.\\
\end{tabularx}
...
\renewcommand\arraystretch{2}% 1.0 is for standard spacing)
\rowcolors{1}{white}{lightgrey}
\begin{tabularx}{\textwidth}{+>{\raggedright\arraybackslash}H^X}
 \hline
\rowstyle{\bfseries}
Directory	&	Description \\ \hline 
\lstinline|/var/www/mouse|	&	\textbf{Base directory} of the whole project relevant files and folders. \\ 
\\
\hline
\multicolumn{2}{|l|}{\textbf{All of the following files and directories are located within the base directory!}} \\
\hline
\\ 
\lstinline|conf/conf.xml|	&	The main \ac{XML} configuration file. \\  
\lstinline|conf/micedata_create.sql|	&	The SQL file used to create the database structure. \\  
\lstinline|import_scripts//|	&	Contains the \ac{BASH} scripts to start the import cascade at a specific script (see \ref{app:import_schedule} on page \pageref{app:import_schedule} for details). \\ 
\lstinline|data/|	&	Uploaded but not yet imported data files. \\ 
\lstinline|data/cronlogs/|	&	Transcripts created the \ac{perl} scripts involved in the data import process. \\  
\lstinline|data/imported/|	&	After successful import of a data file into the database, the file is moved into this directory.  \\  
\lstinline|miceminer/|	&	The \textit{miceminer} application main files and folders.\\ 
\lstinline|perl/cgi|	&	\ac{CGI} scripts written in \ac{perl} which are used by the \textit{miceminer} application. \\ 
\lstinline|perl/extras/|	&	Perl scripts to control the data validity in the database and to compile data for some special analysis. The task of these scripts is usually explained in the header section of the file. \\ 
\lstinline|perl/import/|	&	Contains the perl script involved in the data import and processing. \\ 
\lstinline|perl/lib/|	&	The perl modules needed by all the import, reset and CGI scripts. \\ 
\lstinline|perl/reset/|	&	Scripts to set back the data in the database, so that the import scripts can be rerun. \\ 
\lstinline|amfphp/services/miceMiner/Mice.php|	&	\href{http://www.amfphp.org/}{amfphp} services file. \\  
\lstinline|asdoc/|	&	Contains the \textit{miceminer} \ac{API} online documentation files. \\
\end{tabularx}
...
\lstinline|import_step_1.sh|	&	\lstinline|logimport.pl| \\ 
\lstinline|direction_step_2.sh|	&	\lstinline|searchdir.pl| \\
\lstinline|results_step_3.sh|	&	\lstinline|searchres.pl| \\
\lstinline|meetings_step_4.sh|	&	\lstinline|meetings.pl| \\
\lstinline|counter_step_5.sh|	&	\lstinline|counter.pl| \\ 
\end{tabularx}
...
The main reviewer of the thesis, \textbf{Dr. Thomas Netter}. He contributed numerous corrections and comments and never gave up to improve my writing style. Furthermore, he maintained the server that hosted the initial version of \textit{miceminer},
\vspace{.2cm} 

The reviewers \textbf{Christian Iten} and \textbf{Kaspar M\"uller} for their great job in making this a much better thesis,
\vspace{.2cm}
