...
The chart view for the actual results of a mouse, box or antenna has to be different, as the structure of the data is not appropriate to be shown in a column chart (see figure \ref{fig:overview_data}). Since the reseachers are usually interested at which location a mouse has spent how many time, the data is grouped and displayed as a pie chart (see figure \ref{fig:pie_chart_for_mouse}).

\begin{figure}[htpb]
\begin{center}
  \includegraphics[width=.66\textwidth]{assets/img/pie_chart_for_mouse.png}
  \caption[Pie chart of result data for a mouse]{Exported pie chart with grouped box recordings for a mouse. The chart shows how many times the mouse has been recorded at the antennas of the boxes listed in the legend on the left side (e.g. 4754 times at box 40) durig the selected period.}
  \label{fig:pie_chart_for_mouse}
\end{center}
\end{figure}

When the user has selected the chart view, the \textit{Excel} export behaviour changes. Not the data shown in the table will be exported, but the grouped data underlying the chart. This has been introduced according to the interest of the researchers in this data.

In addition a secondary chart can be requested by the user. If the user double clicks a pie chart item, for example the one for box 40 as shown in figure \ref{fig:pie_chart_for_mouse}, another pie chart will be displayed alongside. In this case, the secondary chart shows which other mice have been recorded in at box 40 during the period. In fact, this secondary chart is shown as well, if the user selects the chart view for the result of box 40 in the same period. Hence, the secondary chart is more of a handy method to view the information at the same time.

Combined with the table view and the shared filter capabilities the visualization options offer a good alternative to explore the data. A special visualization, which shows possible social relations between the mice living in the shed is explained in the following chapter.           
