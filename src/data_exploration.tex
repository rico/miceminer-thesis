...
\subsubsection{Short introduction to network theory}
\label{subsubsec:graph_intro}
...
\paragraph{Network types}
\label{para:net_types}
Depicted in figure \ref{fig:graph_undirected} is a very simple network with five nodes (A to D) and some (binary) edges between them. In social networks, the nodes are normally indviduals and the edges denote a relation between them. For a train network, however, the nodes would stand for the train stations, and the edges for the connecting tracks.
...
\begin{figure}[ht]
    ...
    \vspace{0pt}
		\centering
			...
			\begin{tabularx}{.4\textwidth}{+H|^X^X^X^X}
			\rowstyle{\bfseries}
				&	A	&	B	&	C	&	D \\\midrule
			A	&	0	&	1	&	0	&	1 \\
			B	&	1	&	0	&	1	&	1 \\
...
\begin{figure}[ht]
    ...
    \vspace{0pt}
		\centering
			\newcolumntype{H}{>{\bf}p{.5cm}}
			\begin{tabularx}{.45\textwidth}{+H|^X^X^X^X}
			\rowstyle{\bfseries}
				&	A	&	B	&	C	&	D \\\midrule
			A	&	0	&	5	&	0	&	4 \\
			B	&	5	&	0	&	1	&	2 \\
...
The data matrix corresponding to a visualization, gives us the ability to calculate defined values for nodes, even for large networks, by using a computer. Some of these values are explained in the next section. 

\paragraph{Node based measures}
\label{para:node_based}

In order to get some quantitative data about a node in a network, 
  


\subsubsection{Concept}
\label{subsubsec:graph_concept}

\subsubsection{Implementation}
\label{subsubsec:graph_explore}

\subsubsection{Possible applications}
\label{subsubsec:graph_research}
