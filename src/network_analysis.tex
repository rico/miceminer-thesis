...
\end{tabular}
\captionof{table}{Allocation of the nestboxes, in which meetings occured in January 2009, to the components observed in the network. Additionally the size of the components (number of mice making up the component) and the sum of the meeting durations for the boxes is included (according to \lstinline|<maxStay>| value explained in section \ref{subsec:miceminer_config}, meetings with a duration of stay longer than 9 hours have been excluded from the sum).}
\label{tab:comp_box_meet_dist}
\end{center}
\end{figure} 

Interestingly, if we compare this allocation to the placement of the nestboxes and dividers in the barn (see figure \ref{fig:shedschema} on page \ref{fig:shedschema}), from the six components, five components are restricted to one of the four segments (A, B, C, D). Furthermore, the number of boxes where most of meetings (based on the sum of the stay duration) of a component occur, is between 2 and 4. 
 
\subsubsection{Separation of node based measures by gender}
\label{subsubsec:nbm_dist}

Pictured in figure \ref{fig:node_based:measures_dist} are the distributions of the node based measures for female and male mice. Although the charts do not show a clear separation 
...
Clustering coefficient	&	39	&	30	&  $0.65$ \\
Degree	&	45 	& 	47 	& $0.62$	\\
