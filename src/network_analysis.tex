...
After visually examining the general network structure, the visualizations in figure \ref{fig:graph_january_30h_node_based_measures} are adapted to emphasize the role of the individual nodes in the network, based on the node based measures introduced in section \ref{subsubsec:node_based}. 
...
where $r_i$ is the $r$ value of the network in which the $i$-th edge is removed, to determine the standard deviation, followed by a simple $t$-test\cite{snijders:99}.
